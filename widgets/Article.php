    ...
    if ( ! empty( $settings['category_custom_links'] ) ) {
        foreach ( $settings['category_custom_links'] as $item ) {
            ...
            $posts_per_page = !empty($settings['posts_per_page']) ? intval($settings['posts_per_page']) : 4;
                ...
                echo '<style data-widget="' . esc_attr($widget_id) . '">#' . esc_attr($widget_id) . ' .article-card { visibility: hidden; }</style>';
                ...
                if ($query->have_posts()) {
                    ...
                    while ($query->have_posts()) {
                        ...
                        if ( 'inline-block' === $settings['category_active'] ) { // Corretto il nome della chiave
